...
public class WordLogger {
    ...
    // Each parallel thread gets its own Word document
    private static ThreadLocal<XWPFDocument> document = new ThreadLocal<>();

    // Each parallel thread gets its own output stream
    private static ThreadLocal<FileOutputStream> outputStream = new ThreadLocal<>();

    // Shared folder for the same scenario
    private static volatile String scenarioFolderPath;

    /**
     * Creates/returns the common folder for the scenario.
     */
    public static synchronized void startScenario(String scenarioName, String browser) {
        ...
        try {
            ...
            if (scenarioFolderPath == null) {
                ...
                scenarioFolderPath = baseDir + "/" + folderName;

                File dir = new File(scenarioFolderPath);
            ...
            throw new RuntimeException(
                    "WordLogger init failed: " + e.getMessage(),
                    e
            );
        }
    }

    public static void writeBase64Screenshot(
            String base64,
            String message) {
    ...
    public static void endScenario() {
        ...
        try {
            ...
            // IMPORTANT:
            // Do NOT set scenarioFolderPath = null here.
        }
    }
